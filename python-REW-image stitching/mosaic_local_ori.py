...
def mosaic_local_ori(im1, im2, H, X1_ok, X2_ok):
    ...
    # 参数
    lambd = 0.001 * imsize1[0]*imsize1[1] # 平衡拟合项和平滑项的加权参数。
    intv_mesh = 10 # 用于计算变形函数的像素间隔。
    K_smooth = 5 # 非重叠区域中的平滑过渡宽度被设置为最大偏移的K_smooth倍。
    ...
    # 坐标对齐
    margin = 0.2 * min(imsize1[0],imsize1[1]) # 重新投影图像区域得附加边距.
    ...
    img4 = cv_draw_matches(im1, im2, X1_nd, X2_nd, ok)
    #D:\computer version work\1-image stitching\python-Robust_Elastic_Warping-master\images\J
    cv.imwrite('D:/computer version work/1-image stitching/python-Robust_Elastic_Warping-master/images/B/Bayesian Refinement of Feature Matches.jpg', img4)
    cv.imshow("Bayesian Refinement of Feature Matches", img4)
    cv.waitKey(1)
    # plt.figure()
    ...
    #平滑过渡到全局变换。
    eta_d0 = 0 # 平滑过渡区域的下边界
    eta_d1 = K_smooth * max(abs(np.concatenate([gxn, hyn]))) # 平滑过渡区域的上边界
